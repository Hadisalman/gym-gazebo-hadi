...
# with probability eps. We anneal eps from 0.5 to 0.1 over the course of 1000 steps. This is done so that
...
policy = LinearAnnealedPolicy(EpsGreedyQPolicy(), attr='eps', value_max=.2, value_min=.05, value_test=.05,
                              nb_steps=1e3)
dqn = DQNAgent(model=model, nb_actions=nb_actions, policy=policy, memory=memory,
                nb_steps_warmup=100, gamma=1.00, target_model_update=20,
               enable_dueling_network=True, dueling_type='avg', train_interval=4)

# dqn.compile(RMSprop(lr=.00025), metrics=['mae'])
dqn.compile(RMSprop(lr=.00025), metrics=['mse'])
