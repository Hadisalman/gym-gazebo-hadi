...
class GazeboPath2TurtlebotCameraNnEnv(gazebo_env.GazeboEnv):

    def __init__(self):
        # Launch the simulation with the given launchfile name
        gazebo_env.GazeboEnv.__init__(self, "GazeboPath2TurtlebotLidar_v0.launch")
    ...
    def _step(self, action):
        ...
        if (current_state.pose.position.y > 6.5):
            done = True
    ...
    def _reset(self):
        ...
        try:
            #reset_proxy.call()
            self.reset_proxy()
            self.setmodelstate(x=-0.2,y=0.5,yaw=self.initial_angles[self.episode%2])
